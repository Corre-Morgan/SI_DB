...
Class HorseIssueController{
    ...
    static function load_horse_issue($params){
        $_SESSION['horse_issue'] = Database::get_instance()->query('SELECT * FROM Horse_Issue WHERE id_horse = '.$params['id_horse'].' AND id_issue = '.$params['id_issue']);
    }

    static function load_horse_issues(){
        $_SESSION['horse_issues'] = Database::get_instance()->query('SELECT * FROM Horse_Issue');
    }
