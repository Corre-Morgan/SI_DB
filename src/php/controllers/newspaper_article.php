...
Class NewspaperArticleController{
    ...
    static function load_newspaper_article($params){
        $_SESSION['newspaper_article'] = Database::get_instance($_SESSION['pseudo'], $_SESSION['password'])->query('SELECT * FROM Newspaper_Article WHERE id_newspaper = '.$params['id_newspaper'].' AND id_article = '.$params['id_article']);
    }

    static function load_newspaper_articles(){
        $_SESSION['newspaper_articles'] = Database::get_instance($_SESSION['pseudo'], $_SESSION['password'])->query('SELECT * FROM Newspaper_Article');
    }
