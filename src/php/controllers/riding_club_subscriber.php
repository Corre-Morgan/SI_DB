...
Class RidingClubSubscriberController{
    ...
    static function load_riding_club_subscriber($params){
        $_SESSION['riding_club_subscriber'] = Database::get_instance()->query('SELECT * FROM Riding_club_subscriber WHERE id_riding_club = '.$params['id_riding_club'].' AND id_user = '.$params['id_user']);
    }

    static function load_riding_club_subscribers(){
        $_SESSION['riding_club_subscribers'] = Database::get_instance()->query('SELECT * FROM Riding_club_subscriber');
    }
