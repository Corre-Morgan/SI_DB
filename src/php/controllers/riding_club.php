...
Class RidingClubController{
    ...
    static function load_riding_club($id){
        $_SESSION['riding_club'] = Database::get_instance($_SESSION['pseudo'], $_SESSION['password'])->query('SELECT * FROM Riding_club WHERE id_etablishment ='. $id);
    }

    static function load_riding_clubs(){
        $_SESSION['riding_clubs'] = Database::get_instance($_SESSION['pseudo'], $_SESSION['password'])->query('SELECT * FROM Riding_club');
    }
